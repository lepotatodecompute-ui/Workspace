...
#define ISO_KERNEL_CMD                                                         \
  "console=hvc0 earlyprintk=ttyS0 tsc=unstable "                             \
  "virtio_mmio.device=4K@0x20000000:10 virtio_mmio.device=4K@0x20020000:11 "   \
  "virtio_mmio.device=4K@0x20040000:12 virtio_mmio.device=4K@0x20060000:13 "   \
  "root=/dev/vdb rootfstype=iso9660 ro"
...
static void machine_mem_init(const char *kernel_cmd) {
...
static int64_t g_second_harddisk_sectors = (256LL * 1024 * 1024) / HARDDISK_PAGE;
int64_t second_harddisk_get_sector_count(BlockDevice *bs) {
  return g_second_harddisk_sectors;
}
...
int main(int argc, char **argv) {
  const char *kernel_cmd = KERNEL_CMD;
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "root=vdb") == 0) {
      kernel_cmd = ISO_KERNEL_CMD;
    } else if (strncmp(argv[i], "fs=", 3) == 0) {
      long long filesystem_size_mb = atoll(argv[i] + 3);
      if (filesystem_size_mb > 0) {
        g_second_harddisk_sectors =
            (int64_t)filesystem_size_mb * 1024 * 1024 / HARDDISK_PAGE;
      }
    }
  }
